...
// cstorbackupbuilder defines interface for CStorBackup builder
type cstorbackupbuilder interface {
	WithBackupName(backupname string) *CstorbackupBuilder
	WithVolumeName(volumename string) *CstorbackupBuilder
	WithSnapName(snapname string) *CstorbackupBuilder
	WithPrevSnapName(snapname string) *CstorbackupBuilder
	WithBackupDest(backupdest string) *CstorbackupBuilder
	WithNameSpace(namespace string) *CstorbackupBuilder
	Build() (*CStorBackup, error)
}
...
func (cb *CstorbackupBuilder) WithClientSet(fn LoadBackupClient) *CstorbackupBuilder {
	...
	glog.Errorf("Failed to fetch k8s cluster config. %+v %v", err, cb.obj.client)
	cb.err = err
	return cb
}
...
func (cb *CstorbackupBuilder) BuildFromAPIObject(obj *apis.CStorBackup) (*CStorBackup, error) {
	obj.Name = obj.Spec.SnapName + "-" + obj.Spec.VolumeName
	cb.obj.object = obj
	cb = cb.Validate()
	glog.Infof("bkp :%v %v %v", cb.obj, cb.obj.client, cb.obj.KubeClient)
	return cb.obj, cb.err
}
