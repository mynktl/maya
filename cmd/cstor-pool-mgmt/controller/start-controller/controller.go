...
import (
	...
	restoreinformer "github.com/openebs/maya/pkg/client/generated/openebs.io/restore/v1alpha1/informer/externalversions"
...
func StartControllers(kubeconfig string) {
	...
	if err != nil {
		glog.Fatalf("Error building backup clientset: %s", err.Error())
	}

	restoreClient, err := restoreclientset.NewForConfig(cfg)
	if err != nil {
		glog.Fatalf("Error building restore clientset: %s", err.Error())
	}
	...
	// restoreInformerFactory constructs a new instance of restore sharedInformerFactory.
	restoreInformerFactory := restoreinformer.NewSharedInformerFactory(restoreClient, getSyncInterval())
	...
	backupController := backupcontroller.NewController(kubeClient, backupClient, kubeInformerFactory,
		backupInformerFactory)

	// Instantiate the cStor restore controller
	restoreController := restorecontroller.NewController(kubeClient, restoreClient, kubeInformerFactory,
		restoreInformerFactory)

	go kubeInformerFactory.Start(stopCh)
	go openebsInformerFactory.Start(stopCh)
	go backupInformerFactory.Start(stopCh)
	go restoreInformerFactory.Start(stopCh)
